...
async function main() {
  ...
  await sub.subscribe("orion:bus", async (raw: string) => {
    try {
      ...
      if (m.type === "work") {
        const latest = await memGet<string>(m.taskId, "artifact"); // not strictly needed, demo read
        const review = await mockLLMCritic(latest ?? m.content);
        const critique = buildMsg({ taskId: m.taskId, from: NAME, type: "critique", content: review });
        await pub.publish("orion:bus", JSON.stringify(critique));

        if (/control:done/i.test(review)) {
          const done = buildMsg({ taskId: m.taskId, from: NAME, type: "control", content: "done: artifact meets acceptance" });
